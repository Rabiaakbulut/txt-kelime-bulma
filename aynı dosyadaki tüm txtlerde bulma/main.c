/*
* BLM3010 hafta4 odevi
* @author Rabia Akbulut  170420822   
* 
* Bu program vaynı dosya icindeki butun txtlerde verilen kelimeyi arar.
 ...
 */
void son(char *argv[],char *dosyalar)
{
printf("------------------------------%s\n",dosyalar);
     FILE *fp1 = fopen(dosyalar, "r");  //dosya acma islemleri
     if(fp1 == NULL) {
         perror("Dosya acilamadi");
         return;
     }
     ...
     int kolon=0;
     int sonuc;
     while(fgets(satirdakiYazi, sizeof(satirdakiYazi), fp1) != NULL) { //satır döngüüsü
        kolon=0;
...
altSatiriKontrolEt(kelime,satir,sonuc+1000,dosyalar); //bini cikardik. yukarda acikaldim
break;
}
else{
...
struct dirent *de; 
  ...
  int i=0;//dosya sayisi
	while ((de = readdir(dr)) != NULL)
	{
		strcpy(belge,de->d_name);
		dosyalar[i]=belge;
		printf("******************i.%d dosya-> %s\n",i,dosyalar[i]);
		
		char *word = ".txt";
		char *pch = strstr(dosyalar[i], word); //txt uzantili dosyalara gircez

		if(pch)
		{		
		 son(argv,dosyalar[i]);//kelime arama islemlerine baslanan fonksiyon
		}
		i++;
		kacKereVar=0;
		}
